...
function recursiveLinks(tree, currentSlug, isFirst = true) {
   ...
   const groups = Object.keys(tree)
    ...
    return <ul className="p-0 m-0 list-none list-inside quick-links-div">{children}</ul>
}

const quickLinksSort = (a, b) => {
    if (a.groups && b.groups) {
        const shortestGroup = a.groups.length < b.groups.length ? a.groups.length : b.groups.length
        // strcmp each group
    ...
    let orderA = a.order !== undefined ? a.order : ( a.metadata.order !== undefined ? a.metadata.order : 0)
    let orderB = b.order !== undefined ? b.order : ( b.metadata.order !== undefined ? b.metadata.order : 0)

    let res = orderA - orderB
    if (res !== 0) return res

    // sort by title if order is equal
    let titleA = a.title !== undefined ? a.title : ( a.metadata.title !== undefined ? a.metadata.title : "No title set")
    let titleB = b.title !== undefined ? b.title : ( b.metadata.title !== undefined ? b.metadata.title : "No title set")
